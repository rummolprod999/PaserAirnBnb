...
class Analitycs2Model extends Model
{
    ...
    public function get_data()
    {
        ...
        $query = 'SELECT d.id, day_month, count_m FROM date_not_first_count d';
        if (isset($_GET['date_start'])) {
            $stmt = $this->conn->prepare('SELECT inter.date_start, inter.date_end, DAYOFMONTH(:dt) dd FROM intervals_count inter JOIN date_not_first_count dnf ON inter.id_count = dnf.id WHERE (STR_TO_DATE(:dt, \'%Y-%m-%d\') >= inter.date_start AND STR_TO_DATE(:dt, \'%Y-%m-%d\') <= inter.date_end) AND dnf.day_month = DAYOFMONTH(:dt)');
